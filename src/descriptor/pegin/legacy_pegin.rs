...
impl<Pk: MiniscriptKey> LegacyPegin<Pk> {
    ...
    pub fn new(
        ...
        let fed_ms = BtcMiniscript::from_ast(BtcTerminal::Multi(bitcoin_miniscript::Threshold::new(fed_k, fed_pks.clone()).expect("TODO")))
            .expect("Multi type check can't fail");
        let csv = BtcMiniscript::from_ast(BtcTerminal::Verify(Arc::new(
            BtcMiniscript::from_ast(BtcTerminal::Older(bitcoin_miniscript::RelLockTime::try_from(timelock).expect("TODO"))).unwrap(),
        )))
        .unwrap();
        let emer_ms = BtcMiniscript::from_ast(BtcTerminal::Multi(bitcoin_miniscript::Threshold::new(emer_k, emer_pks.clone()).expect("TODO")))
            .expect("Multi type check can't fail");
        let emer_ms =
    ...
    fn from_ms_and_desc(
        ...
        let (timelock, emer_pks, emer_k) = if let BtcTerminal::AndV(l, r) = right {
            if let (BtcTerminal::Verify(csv), BtcTerminal::Multi(t)) =
                (&l.node, &r.node)
            {
                if let BtcTerminal::Older(timelock) = csv.node {
                    (timelock, t.data().clone(), t.k())
                } else {
                    unreachable!("Only valid pegin miniscripts");
                }
            } else {
